...
export default function BookingPage() {
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    if (res.success) {
      ...
      alert("Došlo je do greške prilikom čuvanja rezervacije. Pokušajte ponovo.");
    }
  };
          ...
          <div className={`space-y-8 ${stepClass}`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-4">
                <h3 className="text-xl font-bold text-padel-blue uppercase tracking-widest pl-2">{pricingData.morning.subtitle}</h3>
                {pricingData.morning.items.map((item, i) => (
                  <button
                    key={i}
                    onClick={() => handlePackageSelect({ ...item, type: 'Pre podne', range: pricingData.morning.range, time: pricingData.morning.subtitle })}
                  ...
                  >
                    <div className="flex justify-between items-center">
                      <div>
                        <span className="block text-white font-bold text-lg">{item.duration}</span>
                        <span className="block text-slate-500 text-xs uppercase">{pricingData.morning.range}</span>
                      </div>
              ...
              <div className="space-y-4">
                <h3 className="text-xl font-bold text-primary-orange uppercase tracking-widest pl-2">{pricingData.afternoon.subtitle}</h3>
                {pricingData.afternoon.items.map((item, i) => (
                  <button
                    key={i}
                    onClick={() => handlePackageSelect({ ...item, type: 'Posle podne', range: pricingData.afternoon.range, time: pricingData.afternoon.subtitle })}
                  ...
                  >
                    <div className="flex justify-between items-center">
                      <div>
                        <span className="block text-white font-bold text-lg">{item.duration}</span>
                        <span className="block text-slate-500 text-xs uppercase">{pricingData.afternoon.range}</span>
                      </div>
      ...
      case 5:
        return (
          <div className={`space-y-8 max-w-xl mx-auto ${stepClass}`}>
            ...
            <form className="p-8 rounded-3xl bg-slate-900/50 border border-white/5 backdrop-blur-xl space-y-4 shadow-2xl" onSubmit={handleFormSubmit}>
              ...
              <button type="button" onClick={prevStep} disabled={isSubmitting} className="w-full mt-4 text-slate-500 font-bold uppercase text-xs tracking-widest hover:text-white transition-colors disabled:opacity-50">
                Nazad
              </button>
            </form>
          </div>
