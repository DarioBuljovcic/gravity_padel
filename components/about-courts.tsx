...
export default function AboutCourts() {
  const features = [
    {
      title: "Plavi WPT Tereni",
      description: "Četiri vrhunska panoramic terena sa plavom podlogom po zvaničnom World Padel Tour standardu.",
      icon: (
    ...
    {
      title: "Night Play",
      description: "Specijalizovano LED osvetljenje koje simulira dnevnu vidljivost bez neprijatnih senki za noćne mečeve.",
      icon: (
        <svg fill="none" viewBox="0 0 24 24" stroke="currentColor" className="w-6 h-6">
    ...
    {
      title: "Relax Zona",
      description: "Uživajte u pogledu sa naše prostrane terase ili se osvežite u moderno opremljenom kafiću.",
      icon: (
        <svg fill="none" viewBox="0 0 24 24" stroke="currentColor" className="w-6 h-6">
        ...
        <div className="relative h-[400px] md:h-[650px] w-full rounded-[2rem] md:rounded-[3rem] overflow-hidden border border-blue-600/10 group scroll-reveal shadow-[0_0_50px_rgba(37,99,235,0.05)]">
          ...
          <div className="absolute bottom-0 left-0 p-8 md:p-12 w-full flex flex-col md:flex-row md:items-end justify-between gap-8 md:gap-10">
            <div className="max-w-2xl text-center md:text-left">
              <span className="text-blue-400 text-[10px] md:text-xs font-black uppercase tracking-[0.4em] mb-3 md:mb-4 block">Zvanični Standard</span>
              <h3 className="text-3xl sm:text-4xl md:text-6xl font-display font-black text-slate-50 leading-[0.9] tracking-tighter">Igra koja nikada <br className="hidden sm:block" />ne spava.</h3>
            </div>
            <Link
              href="https://www.instagram.com/padel.gravity/"
